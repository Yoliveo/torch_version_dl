...
class DiverseLeaner(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(DiverseLeaner, self).__init__(
        ...
        if "teacher_weak" in data_groups:
            ...
            unsup_loss = weighted_loss(
                self.foward_unsup_train(
                    data_groups["teacher_weak"], data_groups["teacher2_weak"], data_groups["unsup_student2_strong"],
                    flag=1
                ),
            ...
            unsup_loss2 = weighted_loss(
                self.foward_unsup_train(
                    data_groups["teacher_weak"], data_groups["teacher2_weak"], data_groups["unsup_student_strong"],
                    flag=2
                ),
                weight=self.unsup_weight,
            )
        ...
        for ind, i in enumerate(sampling_results):
            for jind, j in enumerate(i.pos_assigned_gt_inds):
                if gt_bboxes2[ind][j][4] < 0.9:
                    ...
                    ignore_num += 1
                    student_soft = torch.cat([student_soft, bbox_results['cls_score'][all_num + jind].unsqueeze(0)],
                                             dim=0)  # get the ind th photo's jind th sample
    ...
    def extract_teacher_info(self, img, img_metas, proposals=None, **kwargs):
        ...
        for img_id in range(len(proposal_list)):
            _bbox, _label, _inds = multiclass_nms(proposal_list[img_id],
                                                  ...
                                                  self.teacher.test_cfg.rcnn.nms,
                                                  self.teacher.test_cfg.rcnn.max_per_img, return_inds=True)
            bbox.append(_bbox)
            label.append(_label)
            inds.append(_inds)
    ...
    def extract_teacher2_info(self, img, img_metas, proposals=None, **kwargs):
        ...
        for img_id in range(len(proposal_list)):
            _bbox, _label, _inds = multiclass_nms(proposal_list[img_id],
                                                  ...
                                                  self.teacher2.test_cfg.rcnn.nms,
                                                  self.teacher2.test_cfg.rcnn.max_per_img, return_inds=True)
            bbox.append(_bbox)
            label.append(_label)
            inds.append(_inds)
    ...
    def extract_student_info(self, teacher, img, img_metas, proposals=None, **kwargs):
        ...
        if self.iter >= 80000:
            if self.iter == 80000:
                print("start pb........")
            img = pseudo_label_aware_erasing(pseudo_bboxes, img, lower=0.02, prob=0.5, higher=0.3, ratio=4)
            student_info["img"] = img
            feat = self.student.extract_feat(img)
        else:
            student_info['img'] = img2
            feat = self.student.extract_feat(img2)
    ...
    def extract_student2_info(self, teacher, img, img_metas, proposals=None, **kwargs):
        ...
        if self.iter >= 80000:
            ...
            student_info['img'] = img2
            feat = self.student2.extract_feat(img2)
        student_info["backbone_feature"] = feat
        if self.student2.with_rpn:
    ...
    def pseudo_label_aware_erasing(self, pseudo_bboxes, img, lower, prob, higher, ratio):
