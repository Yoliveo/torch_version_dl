...
class MeanTeacher(Hook):
    ...
    def before_train_iter(self, runner):
        ...
        self.momentum2_update(model,momentum)
    ...
    def momentum2_update(self, model, momentum):
        for (src_name, src_parm), (tgt_name, tgt_parm) in zip(
            model.student2.named_parameters(), model.teacher2.named_parameters()
        ):
            tgt_parm.data.mul_(momentum).add_(src_parm.data, alpha=1 - momentum)
